...
public interface MatchResultRepo extends JpaRepository<MatchResult, Long> {
    ...
    @Transactional
    @Modifying
    @Query("UPDATE MatchResult m SET m.matchForProfileOne = :isMatch, m.resultCompleted = :updatedAt " +
            "WHERE m.matchProfileOne.id= :id1 AND m.matchProfileTwo.id = :id2")
    void updateToMatchResultByMatchProfileOne(@Param("isMatch") Boolean isMatchForMatchProfileOne,
    ...
    @Query("UPDATE MatchResult m SET m.matchForProfileTwo = :isMatch, m.resultCompleted = :updatedAt " +
            "WHERE m.matchProfileOne.id= :id1 AND m.matchProfileTwo.id = :id2")
    void updateToMatchResultByMatchProfileTwo(@Param("isMatch") Boolean isMatchForMatchProfileTwo,
                                              @Param("updatedAt") Instant lastUpdated,
                                              @Param("id1") Long matchProfileOne,
                                              @Param("id2") Long matchProfileTwo);
    ...
    @Query(value = "(select distinct match_profile_id_fk_2 from match_result " +
            "where (match_profile_id_fk_1 = ? " +
            "and match_profile_1_result is not null) " +
            "AND (match_result_completed IS NOT NULL OR (batch_sent is not null and record_expires > ?))) " +
            "union (select distinct match_profile_id_fk_1 from match_result " +
            "where (match_profile_id_fk_2 = ? " +
            "and match_profile_2_result is not null) " +
            "AND (match_result_completed is not null " +
            "OR (batch_sent is not null and record_expires > ?)))", nativeQuery = true)
    List<Long> retrieveAllIdsforMatchProfilesSentInPreviousBatchesAndNotExpired(Long matchProfileId, Instant now, Long matchProfileId1, Instant now1);
